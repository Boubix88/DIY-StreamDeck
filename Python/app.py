...
rgb_values = [255, 255, 255]  # Valeurs RVB initiales
brightness = 100  # Luminosité initiale

# Variable JSON pour les couleurs
colorData = {"Mode" : 1, "R": 255, "G": 255, "B": 255}
...
def apply_changes():
    global ser, rgb_values, brightness
    data = {"Mode" : 1, "R": int(rgb_values[0] * (brightness / 100)), "G": int(rgb_values[1] * (brightness / 100)), "B": int(rgb_values[2] * (brightness / 100))}
    colorData = sd.getColor(data)
...
preview_square = preview_canvas.create_rectangle(0, 0, 100, 50, fill="#000000")
...
for i, color in enumerate(["Rouge", "Vert", "Bleu"]):
    ...
    slider = CTkSlider(fixed_color_tab, from_=0, to=255, number_of_steps=5, command=lambda value, var=var, i=i: update_rgb(i, value, var))
    slider.set(255)  # Initialisation à 255
...
brightness_slider = CTkSlider(fixed_color_tab, from_=0, to=100, number_of_steps=5, command=lambda value, var=brightness_var: set_brightness(value, var))
...
# Bouton "Appliquer" pour envoyer les valeurs
apply_button = customtkinter.CTkButton(fixed_color_tab, text="Appliquer", command=apply_changes)
apply_button.grid(row=5, column=0, columnspan=4, padx=10, pady=10)
