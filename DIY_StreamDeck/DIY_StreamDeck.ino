...
#define TFT_DC 9 // 7 pro micro
#define TFT_CS 10 //14
#define TFT_RST 8

#define NB_LED 14
#define PIN_LED 7 // 9 pro micro
...
void processReceivedData() {
  ...
  /*Serial.print(F("Octets dispo: "));
  Serial.println(Serial.available());
  int peeked = Serial.peek();
  Serial.print(F("Premier octet: "));
  Serial.println(peeked, HEX);*/


  if (Serial.available() > 0) {
    /*Serial.println(F("Data received"));
    String line = Serial.readStringUntil('END');*/
    JsonDocument jsonDocument;
    DeserializationError error = deserializeJson(jsonDocument, Serial);
    /*Serial.print(F("Ligne recue: "));
   ...
   // On vérifie s'il y a un écran dans le JSON
    ...
    static bool lastClearState = false;
    if (jsonDocument.containsKey("clr")) {
      bool currentClearState = jsonDocument["clr"];
      if (currentClearState && currentClearState != lastClearState) {
        clearScreen();
      }
